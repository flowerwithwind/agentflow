#!/usr/bin/env bash
# 在服务器 /opt/agentflow：镜像加速 → pull → up（GitHub Actions deploy.yml 调用）
set -euo pipefail

APP_DIR="${APP_DIR:-/opt/agentflow}"
HEALTH_URL="${HEALTH_URL:-http://127.0.0.1:18182/healthz}"
cd "$APP_DIR"
...
IMAGE="${DOCKER_USERNAME}/agentflow-frontend:${IMAGE_TAG}"
echo "==> Pull $IMAGE"
